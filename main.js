...
import { css as userCss } from './rawfonts/LXGWWenKaiScreen.ttf';
import { css as narrCss } from './rawfonts/LXGWWenKaiScreen.ttf';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    document.body.innerHTML = `<h2 class="error-heading">跨域错误或文件未找到。</h2>`;
  }
...
function initApp() {
  ...
  window.drawTarotCard = function () {
    ...
    wrapper.innerHTML = `
      <div class="tarot-hover-box">
           
        <div class="tarot-flip-container"
             onclick="const inner = this.querySelector('.tarot-inner'); inner.style.transform = inner.style.transform === 'rotateY(180deg)' ? 'rotateY(0deg)' : 'rotateY(180deg)'; window.syncTarotToAI('${randomCard.cn}', '${randomCard.id}', '${cnOrientation}', this);">
          
          <div class="tarot-inner" style="position: relative; width: 100%; height: 100%; transition: transform 0.8s cubic-bezier(0.175, 0.885, 0.32, 1.275); transform-style: preserve-3d;">
            
            <div class="tarot-back" style="position: absolute; width: 100%; height: 100%; backface-visibility: hidden; background: linear-gradient(135deg, #a29bfe, #fd79a8); border: 8px solid #fcfcfc; border-radius: 12px; box-sizing: border-box; display:flex; align-items:center; justify-content:center; box-shadow: inset 0 0 20px rgba(0,0,0,0.1), 0 10px 20px rgba(0,0,0,0.15);">
              <div class="tarot-back-text">?</div>
            </div>
            
            <div style="position: absolute; width: 100%; height: 100%; transform: rotateY(180deg); backface-visibility: hidden; background: #fff; border-radius: 12px; overflow: hidden; border: 8px solid #fcfcfc; box-sizing: border-box; box-shadow: 0 10px 20px rgba(0,0,0,0.2);">
              <img src="tarot/${randomCard.path}" class="tarot-img ${isReversed ? 'tarot-reversed' : ''}">
            </div>
          </div>
        </div>
      </div>

      <p class="tarot-title" style="text-align:center; font-weight: 800; margin-bottom: 5px; font-size: 16px; min-height: 24px;">
        <span id="tarot-title-text" class="tarot-title-text">点击卡牌翻开</span>
      </p>
  ...
  window.syncTarotToAI = function (cnName, enName, orientation, element) {
    ...
    const titleTextEl = document.getElementById('tarot-title-text');
    if (titleTextEl) {
      titleTextEl.style.opacity = '0';
      titleTextEl.style.transform = 'translateY(-8px)';
      setTimeout(() => {
        titleTextEl.innerText = `${cnName} (${orientation})`;
        titleTextEl.style.opacity = '1';
        titleTextEl.style.transform = 'translateY(0)';
      }, 350);
    }
    ...
    document.getElementById('submitScaleBtn').onclick = () => {
      ...
      globalOverlay.innerHTML = `
        <div class="scale-notebook scale-done-notebook">
          ...
          <p class="scale-done-p1">记录已同步给林医生。</p>
          <p class="scale-done-p2">“辛苦了，接下来交给我吧。”</p>
  ...
  function generatePlant() {
    ...
    function addFlower(x, y, angle, scale, delay) {
      ...
      if (cfg.fType === 'peach') {
        ...
        fHtml += rE(0, -8, 6, 3, '#FFD700', null, 0, 1, 0.85);
        ...
        let stamen = rc.path('M0,0 Q5,-15 0,-28', {
          stroke: '#FFD700',
          strokeWidth: 2,
          roughness: 1.5,
        });
        ...
        for (let i = 0; i < 4; i++) {
          fHtml += rP('M0,0 Q-8,-15 0,-20 Q8,-15 0,0 Z', c1, c1, i * 90);
        }
  ...
  let playlist = filesMap[timePeriod].map((filename) => {
    ...
    return {
      artist: info.artist,
      name: info.title,
      src: `${import.meta.env.BASE_URL}music/${timePeriod}/${encodeURIComponent(filename)}`,
    };
  });
  ...
  window.activateBirthdayMode = function () {
    ...
    const bdayTracks = [
      ...
      src: `${import.meta.env.BASE_URL}music/birthday/${encodeURIComponent(f)}`,
    }));
  ...
  async function loadAndShowPresent() {
    ...
    container.innerHTML = `<div class="present-loading">正在打包礼物...</div>`;
    ...
    try {
      ...
      container.innerHTML = `<div class="present-error">礼物被快递弄丢了</div>`;
    }
  }
  ...
  function loadTrack(index, autoStart = false, isPrev = false) {
    ...
    setTimeout(() => {
      [trackArtist, trackName].forEach((el) => {
  ...
  window.devTestStream = function () {
    ...
    bubble.className = 'ai-msg msg-narration dev-msg-narration';
    box.appendChild(bubble);

    let text = `林雨涵推开那扇虚掩的橡木门，目光穿透屏幕直视着你。
...
“L:小葵，带弟弟去沙盘室玩。”林雨涵无奈地叹了口气，“L:如你所见，这就是日常。”`;
    ...
    function typeNext() {
      if (index <= text.length) {
        ...
        let htmlOutput = window.parseMD(window.applyCharacterFonts(currentText));

        if (htmlOutput.trim().endsWith('</p>')) {
          bubble.innerHTML = htmlOutput.replace(
            /<\/p>\s*$/,
            '<span class="typing-cursor"></span></p>',
          );
        } else {
          bubble.innerHTML = htmlOutput + '<span class="typing-cursor"></span>';
        }
        ...
        if (Math.random() < 0.05) delay += Math.random() * 150 + 100;
        streamTimeout = setTimeout(typeNext, delay);
      } else {
        bubble.innerHTML = window.parseMD(window.applyCharacterFonts(text));
        streamTimeout = null;
      }
    }
    typeNext();
...
秦渡言站起身，走之前回头看了你一眼，语气里带着难得的温和：『Q:不管你是在测试 \`marked.js\`，还是在看我们忙活...谢谢你愿意听我说。』`;
    ...
    const parsedText = window.parseMD(window.applyCharacterFonts(rawAiText));

    box.innerHTML = `
      <div class="user-note msg-user dev-msg-user">
        <p>我已经阅读了你的 <code>&lt;character_秦渡言_基础人设&gt;</code> 文件。渲染测试正式开始。</p>
      </div>
      <div class="ai-msg msg-narration dev-msg-narration">${parsedText}</div>
    `;
    box.scrollTop = box.scrollHeight;
  };
...
window.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'SYNC_CHAT') {
    ...
    if (existingNodes.length > msgs.length) {
      for (let i = msgs.length; i < existingNodes.length; i++) {
  ...
  if (event.data.type === 'STREAM_UPDATE') {
    ...
    let htmlOutput = window.parseMD(window.applyCharacterFonts(rawText.trim()));
    if (htmlOutput.trim().endsWith('</p>')) {
      typingBubble.innerHTML = htmlOutput.replace(
        /<\/p>\s*$/,
        '<span class="typing-cursor"></span></p>',
      );
    } else {
