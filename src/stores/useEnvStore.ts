...
export const useEnvStore = defineStore('envStore', () => {
  ...
  const fetchHolidays = async (countryCode: string) => {
    ...
    let parsedHolidays: Record<string, string> = {};
  ...
  const syncEnvDataToWorldbook = async (enabled: boolean) => {
    if (!enabled) {
      await removeWorldbookEntry();
      return;
    }
    ...
    try {
      ...
      alert(e.message || '同步环境数据失败，请检查网络。');
      throw e;
    } finally {
  ...
  const removeWorldbookEntry = async () => {
    const chatStore = useChatStore();
